...
#                 # reference2 = '/mnt/c/Users/maaik/OneDrive/Documents/UU/MinorResearchProject/Stefan/shellCommandTest/RealWrapperData/infant_2yr_intensity_1mm.nii.gz'
#                 # reference2 = 'template_v2/infant-2yr-intensity-3mm.nii'
#                 # reference2 = 'MNI152_T1_2mm_brain'
#                 configuration = '--config=parameters_test'
#                 # myCommand = f'fnirt --in="{output_extracted}" --ref="{reference2}" --aff="{affine_output}" --cout="{warp_output}" --fout="{warpfield_output}" --iout="{output_warped_structural}"'
...
#                 # myCommand = f'fnirt --in="{output_extracted}" --ref="{reference2}" --aff="{affine_output}" --cout="{warp_output}" {configuration} --fout="{warpfield_output}" --iout="{output_warped_structural}"'
#                 print('Running my Command:', myCommand)
#                 os.system(myCommand)
#                 print('My Command completed:', myCommand)

#             # # Apply the registration field to the image
#             # if not os.path.exists(output_warped_structural):
#             #     myCommand = f'applywarp {reference2} --in="{output_extracted}" --warp="{warp_output}" --out="{output_warped_structural}"'
#             #     print('Running my Command:', myCommand)
#             #     os.system(myCommand)
#             #     print('My Command completed:', myCommand)
...
###IMAGE QUALITY ANALYSIS
    ...
    if os.path.exists(folder_path):
        ...
        for file_name in os.listdir(folder_path):
            ...
            if file_name.endswith('_struct.nii.gz'):
                ...
                #Lowfield
                # files_outskin.append('Lowfield_fMRItest/average_T1_12MO_air_mask.nii.gz')
                ...
                atlas_file = 'infant-1yr-aal-1mm.nii'
                files_atlas.append('infant-1yr-aal-1mm.nii')
                # files_atlas.append('AAL.nii')

                namelist = file_name.split("_")
                identifier = namelist[2]
                new_path = os.path.join(output_folder2, identifier+'_snrZmap.nii')
                new_path2 = os.path.join(output_folder2, identifier+'_snrDZmap.nii')
    ...
    for i, file in enumerate(files_list):
        ...
        if not files_exist:
            ...
            write_qmfile(file_names[i],output_folder2,calculate_cjv(img_data,wm_mask,gm_mask),calculate_cnr(img_data,wm_mask,gm_mask,std_air),art_qi1(air_img,art_img),art_qi1(air_img,art_img)+art_qi2(noNeck_data,air_img),sum_def)
